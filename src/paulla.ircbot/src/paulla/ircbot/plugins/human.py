...
class Human(object):
    ...
    def on_message(self, mask=None, event=None, target=None, data=None):
        no_human = self.bot.config['paulla.ircbot.plugins.human']['nohuman']
        human = True
        for bot in no_human:
            if md5(bytes(mask.nick[0:8].lower().encode('utf-8'))).hexdigest() == "612143036abea252012f5acd98779ced":
                message = """
                %s: Je parle pas le %s, si tu veux me l'apprendre ca se passe ici : https://github.com/paulla/botanik/blob/master/paulla.ircbot/src/paulla/ircbot/plugins/human.py#L70
                """ % (mask.nick, mask.nick)
                self.call_with_human_delay(self.bot.privmsg, target, message)
                human = None
                break
            if bot.lower() in mask.lower():
                human = None
                break
        if human:
